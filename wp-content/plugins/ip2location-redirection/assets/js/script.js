jQuery(document).ready(function($) {
    ...
    $('#update_ip2location_database').on('click', function(e) {
		...
		if ($('#download_token').val().length > 0) {
			...
			$('#download_status').html('<span class="dashicons dashicons-update spin"></span> Downloading IP2Location DB3 database...');

			$.post(ajaxurl, { action: 'ip2location_redirection_download_database', database: 'DB3' }, function(response) {
				...
				if (response == 'SUCCESS') {
					...
					if (response.includes('do not have')) {
						$('#download_status').html('<span class="dashicons dashicons-update spin"></span> Downloading IP2Location DB1 database...');

						$.post(ajaxurl, { action: 'ip2location_redirection_download_database', database: 'DB1' }, function(response) {
							response = response.replace(/^\s+|\s+$/g, '');
				...
				.error(function() {
					$('#download_status').html('<span class="dashicons dashicons-update spin"></span>Not enough resource for DB3. Downloading IP2Location DB1 database...');
    ...
    function insert_rule(country_codes, from, to, url_from, url_to, wpml_code, http_code, status) {
        var countries = {
            "": "",
            "AF": "Afghanistan",
            "AL": "Albania",
		...
		var $status = $('<select name="rule_status[]">').html('<option value="0"' + ((status == '0') ? ' selected' : '') + '> Inactive</option><option value="1"' + ((status == '1') ? ' selected' : '') + '> Active</option>');
        ...
        if (typeof(rules) !== 'undefined') {
            $.each(rules, function(i, row) {
                if (row.length == 5) {
                    ...
                    insert_rule(row[0], row[1], row[2], row[3], row[4], row[5], 1);
                } else if (row.length == 7) {
					insert_rule(row[0], row[1], row[2], row[3], row[4], '', row[5], row[6]);
				} else {
					insert_rule(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7]);
				}
            });
            insert_rule('', '', '', '', '', '', 301, 1);
        }
	...
	$('#btn-to-step-2').on('click', function(e) {
		...
		$.post(ajaxurl, { action: 'ip2location_redirection_validate_token', token: $('#setup_token').val() }, function(response) {
			response = response.replace(/^\s+|\s+$/g, '');
